...
[](#VSCodeFormatting-fig) shows a screenshot after adding a couple of lines of poorly formatted or anti-pattern code:

```{figure} images/code_formatting.png
...
[](#AIFormatting-fig) shows a simple example where we prompt the GitHub Copilot chat within VSCode to fix the formatting in the example code from above.

```{figure} images/code_formatting_ai.png
...
A better practice here would be to raise an *exception*.
...
In 2013, a group of researchers published a paper in the journal *PLOS One* reporting a relationship between belief in conspiracy theories and rejection of science [@Lewandowsky:2013aa].
...
- Linus' law ("given enough eyeballs, all bugs are shallow") [@Raymond:1999aa] suggests that highly used software is less likely to retain bugs 
